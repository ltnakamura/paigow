...
@synthesize questionTile, drawnTiles, answerStatus, isPositionMode, correct, answerExplain, tileText;
/*
...
-(IBAction) onNewQuestionClick {
	...
	if([self isPositionMode]) {
		...
		questionTile.text = [NSString stringWithFormat:@"Which of these has a Tile Rank %d?", questionDomino.position] ;
		...
		questionTile.text = [NSString stringWithFormat:@"Which of these tiles is a %@ tile?", question.shortName];
		questionAnswerPos = index;
	}
		...
		if([aSubview pointInside: [self.view convertPoint:touchLocation toView:aSubview] withEvent:event]) {
			...
			if(numAttempts < 3) {
				...
				if(answer == questionAnswerPos) {
					answerStatus.text = [NSString stringWithFormat:@"Correct! that is a %@, (Rank %d)", selected.shortName, selected.position];
					nextQuestion.hidden = false;
					numAttempts = 4;
					
				} else {
					aSubview.alpha = 0.5;
					answerStatus.text = [NSString stringWithFormat:@"Incorrect! that is a %@, (Rank %d), try again", selected.shortName, selected.position];
					numAttempts ++;
				}
				...
				if(numAttempts > 2) {
					...
					for (UIView* aSubview2 in self.view.subviews) {
						...
						if(questionDomino.position == GEEJOON) {
