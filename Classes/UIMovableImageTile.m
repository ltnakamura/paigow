...
- (id)initWithHands:(Hand*) upper lowerHand:(Hand*) lower {
	...
	if (self = [super initWithFrame:imageFrame]) {
		...
		Domino* drawnTile;
		UIImage* image;
        NSString *tileImageFileName;
        
		// show high hand
		drawnTile = (Domino *) upper.tileOne;
		tileImageFileName = [drawnTile.name stringByAppendingString:@".png"];
		image = [self getScaledImage:[UIImage imageNamed:tileImageFileName] scaleBy:0.09f];
		UIImage* rotatedImg = [self rotateImg:image orientation:UIImageOrientationLeft];
		CGContextDrawImage(context, CGRectMake(xOrig0, xOrig0 + 12,image.size.height , image.size.width), rotatedImg.CGImage);
		
		drawnTile = (Domino *) upper.tileTwo;
		tileImageFileName = [drawnTile.name stringByAppendingString:@".png"];
		image = [self getScaledImage:[UIImage imageNamed:tileImageFileName] scaleBy:0.09f];
		rotatedImg = [self rotateImg:image orientation:UIImageOrientationLeft];
		CGContextDrawImage(context, CGRectMake(xOrig0, xOrig0 + xWidth + 13 ,image.size.height , image.size.width), rotatedImg.CGImage);
        
 		// show low hand
        drawnTile = (Domino *) lower.tileOne;
        tileImageFileName = [drawnTile.name stringByAppendingString:@".png"];
		image = [self getScaledImage:[UIImage imageNamed:tileImageFileName] scaleBy:0.09f];
		
		CGContextDrawImage(context, CGRectMake(xOrig0 + image.size.height, xOrig0 + 12 ,image.size.width , image.size.height), image.CGImage);
		
		drawnTile = (Domino *) lower.tileTwo;
		tileImageFileName = [drawnTile.name stringByAppendingString:@".png"];
		image = [self getScaledImage:[UIImage imageNamed:tileImageFileName] scaleBy:0.09f];
		CGContextDrawImage(context, CGRectMake(xOrig0 + image.size.height + xWidth, xOrig0 + 12 ,image.size.width , image.size.height), image.CGImage);
		
		
		//CGContextSetRGBFillColor(context, 0.5, 0.5, 0.5, 1);
		NSString* dispString = [NSString stringWithFormat:@"%@ | %@", [upper getHandDisplay],[lower getHandDisplay]];
        
//        [dispString drawAtPoint:CGPointMake(5.0f, 0.0f) withAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"Arial"size:6]}];
        ...
        CGSize txtsize = [dispString sizeWithAttributes:@{NSFontAttributeName:font}];
        [dispString drawInRect:CGRectIntegral(CGRectMake(5, 20, txtsize.width, txtsize.height)) withAttributes:@{NSFontAttributeName:font}];
